# Papyrus Workspace Files for Alfonso (Modeler)
## Section 3.3: Object Interaction - Sequence Diagrams
...
```
C:\Users\alewi\workspace-papyrus\MenuMap_UML_Design\
```
...
1. **UC-001: Browse Restaurant Menus** (Interaction_Interaction4_)
2. **UC-002: Manage Favorites** (Interaction_Interaction4_)
3. **UC-003: Secure Password Reset** (Interaction_Interaction1_)
4. **UC-004: Registration** (Interaction_Interaction1_)
5. **UC-005: User Login** (Interaction_Interaction1_)
6. **UC-006: User Logout** (Interaction_Interaction1_)
7. **UC-007: Menu Management** (Interaction_Interaction1_)
...
1. Open Papyrus
2. File → Open Model → Navigate to `MenuMap_UML_Design.aird`
3. The workspace will load with all diagrams

### **To Find Sequence Diagrams:**
1. In Model Explorer, look for "Interaction" elements
2. The 7 sequence diagrams are stored as Interaction elements
3. Double-click any Interaction to open its sequence diagram editor

### **What Alfonso Needs to Do for Section 3.3:**
- Refine the existing sequence diagrams from SRD
- Add additional "solution objects" (if needed)
...
- Alfonso can copy it to his Papyrus workspace location
...
- Alfonso can extract and place in his Papyrus workspace
...
3. **PNG exports are separate** - The PNG files are just exports, not the editable source
4. **Papyrus workspace location** - Alfonso may need to adjust the workspace path in Papyrus preferences
...
```
C:\Users\alewi\OneDrive\Desktop\MenuMap_CEN4010\Deliverable2_Design\UML_Diagrams\
```
These contain detailed descriptions of each sequence diagram's message flow.

---

**Created:** For Alfonso Oramas (Modeler) - Section 3.3 Object Interaction
